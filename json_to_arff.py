...
def format_hard_sensors_data(letter, key):
    line = ""
    for coord in "xyz":
        i = 0
        while i < 5:
...
def format_hard_sensors_attributes(letter):
    line = ""
    for coord in "XYZ":
        i = 0
        while i < 5:
...
def write_attributes_data(fd, json_data, params):
    for letter in params:
        if letter in "lmno":
            line = format_hard_sensors_attributes(letter)
        else:
    ...
    for section in json_data:
        for key in json_data[section]:
            ...
            for letter in params:
                if letter in "lmno":
                    line += format_hard_sensors_data(letter, key)
                else:
